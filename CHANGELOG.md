...
- A `?theme=light|dark` query param forces that rendition for the view without touching the stored preference - shareable forced-theme links, and what the new screenshot harness (`scripts/theme_shots.sh`: standard page set, two viewports, both renditions, print PDF of `/cv/`) drives for theme QA. (#132)
