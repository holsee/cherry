...
### Added
- Every official theme gains a "Using THEME" page on cherrybomb.dev (`/themes/NAME/`): install, the styling ladder applied to that theme, the full token API with light/dark values and roles, the shipped-vs-inherited template split, and how to point a coding agent at the Cherry way. Generated from each theme's own manifest by `scripts/gen_theme_docs.exs`, so the pages cannot drift; the gallery links each section to its page, the landing page shows six theme cards right after Plant, and the checklist links the gallery. (#142)
