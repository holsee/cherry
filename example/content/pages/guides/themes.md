...
Twenty ship built-in. The quiet ones: `default` (typography-first, Charter serif), `cherrybomb` (the brand, and the one this site wears), `porcelain` (glazed ceramic, Literata), `teletype` (all-mono devlog), `broadsheet` (a newspaper masthead and front page), `halftone` (risograph dot screens and poster cards), `brutalist` (3px borders and Anton), `bento` (product-page tiles), `isometric` (a left rail over a perspective grid floor) and `cathode` (a CRT, scanlines and all). The moving ones: `prism` (a WebGL gradient mesh under glass), `constellation` (a star field), `warp` (light streaks), `erosion` (grains on a flow field), `vortex` (kinetic type on text rings), `sylva` (a season of leaves, petals or snow), `tideform` (layered swell), `kage` (cinematic scroll scenes), `cinema` (a looping video hero) and `showoff` (all of it at once, on purpose). Every one is built on the same token API; most ship a template or two and inherit the rest from the default theme (contract 1.1). See them side by side in the [gallery](/themes/) or live at the [exhibition](https://themes.cherrybomb.dev). Inspect any theme's full API:
