---
title: "Using topo"
description: "Install, override, and extend the topo theme the Cherry way: its token API, its templates, and where each rung of the styling ladder applies."
---
## Using topo
...
### The styling ladder, applied to topo

Restyling costs exactly as much ownership as you choose to take. In order:

**Rung 1 - override a token.** The manifest below is topo's public styling API. Overrides land in every page head, survive theme upgrades, and are validated against the manifest (a typo is an error, not a dead line):

```text
$ cherry config tokens.--color-accent "light-dark(#b5451b, #e0895a)"
$ cherry theme.tokens        # shows the merged view
```

**Rung 2 - custom.css.** Drop `assets/custom.css` into your site and it links after the theme stylesheet and the tokens block on every page. topo's own CSS lives in the `theme` cascade layer, so a plain selector in your unlayered file beats the theme without specificity games or `!important`.

**Rung 3 - overlay a template.** Write `themes/topo/templates/<template>.html.eex` (or `.html.heex`) in your site and it shadows just that template.
...
**Rung 5 - a theme of your own.** `cherry gen.theme mytheme --from topo` scaffolds the whole thing - manifest, templates, stylesheet, islands - into `themes/mytheme/`, yours deliberately.
...
| --- | --- | --- | --- |
| `--color-bg` | `#eef0e4` | `#151a12` | Page background. |
| `--color-surface` | `#e3e6d6` | `#1d2419` | Raised ground: code blocks, inline code. |
| `--color-fg` | `#2b2418` | `#ece8da` | Body text. |
| `--color-muted` | `#6b6350` | `#a6a18c` | Secondary text: metadata, nav, footer. |
| `--color-border` | `#cfd2c0` | `#2e3628` | Hairline rules and control borders. |
| `--color-accent` | `#b5451b` | `#e0895a` | Links and interactive accents. |
| `--color-accent-strong` | `#8f3514` | `#f0aa84` | Hover/active accent. |
| `--color-selection` | `#f3d7c8` | `#4a3322` | Text selection ground. |

#### Syntax highlighting

| Token | Light | Dark | Role |
| --- | --- | --- | --- |
| `--syn-keyword` | `#b5451b` | `#e0895a` | Syntax: keywords. |
| `--syn-string` | `#3e6b2a` | `#9ccf7e` | Syntax: strings and characters. |
| `--syn-comment` | `#87846f` | `#7c8270` | Syntax: comments (italic). |
| `--syn-function` | `#6a3d9a` | `#c8a6f0` | Syntax: functions and methods. |
| `--syn-constant` | `#9a5f00` | `#f0c070` | Syntax: constants, numbers, booleans. |
| `--syn-type` | `#2a5f7a` | `#7fc4d6` | Syntax: types, modules, tags, attributes. |
| `--syn-variable` | `#2b2418` | `#ece8da` | Syntax: variables and default code text. |
| `--syn-punct` | `#6b6350` | `#a6a18c` | Syntax: punctuation and operators. |

#### Type and measure

| Token | Default | Role |
| --- | --- | --- |
| `--font-prose` | `'Alegreya', Georgia, 'Times New Roman', serif` | Reading face (Alegreya, self-hosted latin subsets, OFL). Structure uses Alegreya Sans. |
...
topo ships 1 of the 9 templates in the contract; the rest resolve to the default theme's copy, so they stay current without you owning them. Overlay or eject any name in either list.
...
- *"Warm up topo's palette for both renditions"* - the agent lists the manifest with `cherry theme.tokens`, overrides rung-1 tokens, and verifies with `cherry check`.
...
The one rule to hold an agent to: never edit the theme's own files under `themes/topo/` unless they were ejected there on purpose - `cherry theme.diff` will name anything that drifts.
