---
title: "Using sketchbook"
description: "Install, override, and extend the sketchbook theme the Cherry way: its token API, its templates, and where each rung of the styling ladder applies."
---
## Using sketchbook
...
```text
$ cherry config theme sketchbook
```
...
### The styling ladder, applied to sketchbook

Restyling costs exactly as much ownership as you choose to take. In order:

**Rung 1 - override a token.** The manifest below is sketchbook's public styling API. Overrides land in every page head, survive theme upgrades, and are validated against the manifest (a typo is an error, not a dead line):

```text
$ cherry config tokens.--color-accent "light-dark(#1f5fbf, #8ab4ff)"
$ cherry theme.tokens        # shows the merged view
```

**Rung 2 - custom.css.** Drop `assets/custom.css` into your site and it links after the theme stylesheet and the tokens block on every page. sketchbook's own CSS lives in the `theme` cascade layer, so a plain selector in your unlayered file beats the theme without specificity games or `!important`.

**Rung 3 - overlay a template.** Write `themes/sketchbook/templates/<template>.html.eex` (or `.html.heex`) in your site and it shadows just that template.
...
**Rung 5 - a theme of your own.** `cherry gen.theme mytheme --from sketchbook` scaffolds the whole thing - manifest, templates, stylesheet - into `themes/mytheme/`, yours deliberately.
...
| --- | --- | --- | --- |
| `--color-bg` | `#fcfbf7` | `#1b1f22` | Page background. |
| `--color-surface` | `#f1efe8` | `#252a2e` | Raised ground: code blocks, inline code. |
| `--color-fg` | `#22252b` | `#eeeae0` | Body text. |
| `--color-muted` | `#646a75` | `#a7aba8` | Secondary text: metadata, nav, footer. |
| `--color-border` | `#d7d5cc` | `#3a4045` | Hairline rules and control borders. |
| `--color-accent` | `#1f5fbf` | `#8ab4ff` | Links and interactive accents. |
| `--color-accent-strong` | `#174a96` | `#b3cdff` | Hover/active accent. |
| `--color-selection` | `#d9e6fb` | `#2d4269` | Text selection ground. |

#### Syntax highlighting

| Token | Light | Dark | Role |
| --- | --- | --- | --- |
| `--syn-keyword` | `#1f5fbf` | `#8ab4ff` | Syntax: keywords. |
| `--syn-string` | `#2a7a4b` | `#8fd5a4` | Syntax: strings and characters. |
| `--syn-comment` | `#8a8e98` | `#7f8589` | Syntax: comments (italic). |
| `--syn-function` | `#8a3fb5` | `#d4a6ff` | Syntax: functions and methods. |
| `--syn-constant` | `#c2511e` | `#ffab7a` | Syntax: constants, numbers, booleans. |
| `--syn-type` | `#0f6e8c` | `#79cfe6` | Syntax: types, modules, tags, attributes. |
| `--syn-variable` | `#22252b` | `#eeeae0` | Syntax: variables and default code text. |
| `--syn-punct` | `#646a75` | `#a7aba8` | Syntax: punctuation and operators. |

#### Type and measure

| Token | Default | Role |
| --- | --- | --- |
| `--font-prose` | `'Atkinson Hyperlegible Next', system-ui, 'Segoe UI', Roboto, sans-serif` | Reading face (Atkinson Hyperlegible Next, self-hosted latin subsets, OFL). Display uses Shantell Sans. |
...
sketchbook ships 1 of the 9 templates in the contract; the rest resolve to the default theme's copy, so they stay current without you owning them. Overlay or eject any name in either list.

#### Shipped by sketchbook
...
- *"Warm up sketchbook's palette for both renditions"* - the agent lists the manifest with `cherry theme.tokens`, overrides rung-1 tokens, and verifies with `cherry check`.
...
The one rule to hold an agent to: never edit the theme's own files under `themes/sketchbook/` unless they were ejected there on purpose - `cherry theme.diff` will name anything that drifts.
