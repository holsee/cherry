---
title: "Using bento"
description: "Install, override, and extend the bento theme the Cherry way: its token API, its templates, and where each rung of the styling ladder applies."
---
## Using bento
...
### The styling ladder, applied to bento

Restyling costs exactly as much ownership as you choose to take. In order:

**Rung 1 - override a token.** The manifest below is bento's public styling API. Overrides land in every page head, survive theme upgrades, and are validated against the manifest (a typo is an error, not a dead line):

```text
$ cherry config tokens.--color-accent "light-dark(#0066cc, #2997ff)"
$ cherry theme.tokens        # shows the merged view
```

**Rung 2 - custom.css.** Drop `assets/custom.css` into your site and it links after the theme stylesheet and the tokens block on every page. bento's own CSS lives in the `theme` cascade layer, so a plain selector in your unlayered file beats the theme without specificity games or `!important`.

**Rung 3 - overlay a template.** Write `themes/bento/templates/<template>.html.eex` (or `.html.heex`) in your site and it shadows just that template.
...
**Rung 5 - a theme of your own.** `cherry gen.theme mytheme --from bento` scaffolds the whole thing - manifest, templates, stylesheet - into `themes/mytheme/`, yours deliberately.
...
| `--color-surface` | `#ffffff` | `#1c1c1e` | Raised ground: code blocks, inline code. |
| `--color-fg` | `#1d1d1f` | `#f5f5f7` | Body text. |
| `--color-muted` | `#6e6e73` | `#a1a1a6` | Secondary text: metadata, nav, footer. |
| `--color-border` | `#e5e5ea` | `#2c2c2e` | Hairline rules and control borders. |
| `--color-accent` | `#0066cc` | `#2997ff` | Links and interactive accents. |
| `--color-accent-strong` | `#004a99` | `#6fb8ff` | Hover/active accent. |
| `--color-selection` | `#d2e6ff` | `#143a66` | Text selection ground. |

#### Syntax highlighting

| Token | Light | Dark | Role |
| --- | --- | --- | --- |
| `--syn-keyword` | `#0066cc` | `#2997ff` | Syntax: keywords. |
| `--syn-string` | `#1f7a4d` | `#6fd39a` | Syntax: strings and characters. |
| `--syn-comment` | `#8e8e93` | `#8e8e93` | Syntax: comments (italic). |
| `--syn-function` | `#6e34c9` | `#c3a3ff` | Syntax: functions and methods. |
| `--syn-constant` | `#c2410c` | `#ffa46b` | Syntax: constants, numbers, booleans. |
| `--syn-type` | `#a0127a` | `#ff8ad4` | Syntax: types, modules, tags, attributes. |
| `--syn-variable` | `#1d1d1f` | `#f5f5f7` | Syntax: variables and default code text. |
| `--syn-punct` | `#6e6e73` | `#a1a1a6` | Syntax: punctuation and operators. |

#### Type and measure

| Token | Default | Role |
| --- | --- | --- |
| `--font-prose` | `'Onest', system-ui, 'Segoe UI', Roboto, sans-serif` | Reading and display face (Onest, self-hosted 30 KB latin subset, OFL). |
...
bento ships 3 of the 9 templates in the contract; the rest resolve to the default theme's copy, so they stay current without you owning them. Overlay or eject any name in either list.
...
- *"Warm up bento's palette for both renditions"* - the agent lists the manifest with `cherry theme.tokens`, overrides rung-1 tokens, and verifies with `cherry check`.
...
The one rule to hold an agent to: never edit the theme's own files under `themes/bento/` unless they were ejected there on purpose - `cherry theme.diff` will name anything that drifts.
