---
title: "Using kage"
description: "Install, override, and extend the kage theme the Cherry way: its token API, its templates, and where each rung of the styling ladder applies."
---
## Using kage
...
### The styling ladder, applied to kage

Restyling costs exactly as much ownership as you choose to take. In order:

**Rung 1 - override a token.** The manifest below is kage's public styling API. Overrides land in every page head, survive theme upgrades, and are validated against the manifest (a typo is an error, not a dead line):

```text
$ cherry config tokens.--color-accent "light-dark(#8a6a1f, #c9a45c)"
$ cherry theme.tokens        # shows the merged view
```

**Rung 2 - custom.css.** Drop `assets/custom.css` into your site and it links after the theme stylesheet and the tokens block on every page. kage's own CSS lives in the `theme` cascade layer, so a plain selector in your unlayered file beats the theme without specificity games or `!important`.

**Rung 3 - overlay a template.** Write `themes/kage/templates/<template>.html.eex` (or `.html.heex`) in your site and it shadows just that template.
...
**Rung 5 - a theme of your own.** `cherry gen.theme mytheme --from kage` scaffolds the whole thing - manifest, templates, stylesheet, islands - into `themes/mytheme/`, yours deliberately.
...
| --- | --- | --- | --- |
| `--color-bg` | `#f4f2ec` | `#0b0b0c` | Page background. |
| `--color-surface` | `#ebe8df` | `#151516` | Raised ground: code blocks, inline code. |
| `--color-fg` | `#141413` | `#ecebe6` | Body text. |
| `--color-muted` | `#5f5c55` | `#9b9991` | Secondary text: metadata, nav, footer. |
| `--color-border` | `#dcd8cc` | `#26262a` | Hairline rules and control borders. |
| `--color-accent` | `#8a6a1f` | `#c9a45c` | Links and interactive accents. |
| `--color-accent-strong` | `#6a4f12` | `#e3c484` | Hover/active accent. |
| `--color-selection` | `#efe2bb` | `#3d3320` | Text selection ground. |

#### Syntax highlighting

| Token | Light | Dark | Role |
| --- | --- | --- | --- |
| `--syn-keyword` | `#8a6a1f` | `#c9a45c` | Syntax: keywords. |
| `--syn-string` | `#2f6b4a` | `#8fd0a8` | Syntax: strings and characters. |
| `--syn-comment` | `#8a877e` | `#76756f` | Syntax: comments (italic). |
| `--syn-function` | `#6a3f9e` | `#c6a8ff` | Syntax: functions and methods. |
| `--syn-constant` | `#1f5a9e` | `#8ab8ff` | Syntax: constants, numbers, booleans. |
| `--syn-type` | `#9a3c2a` | `#ff9a86` | Syntax: types, modules, tags, attributes. |
| `--syn-variable` | `#141413` | `#ecebe6` | Syntax: variables and default code text. |
| `--syn-punct` | `#5f5c55` | `#9b9991` | Syntax: punctuation and operators. |

#### Type and measure

| Token | Default | Role |
| --- | --- | --- |
| `--font-prose` | `'Hanken Grotesk', system-ui, 'Segoe UI', Roboto, sans-serif` | Reading face (Hanken Grotesk, self-hosted 27 KB latin subset, OFL). Titles use Instrument Serif (28 KB). |
...
kage ships 2 of the 9 templates in the contract; the rest resolve to the default theme's copy, so they stay current without you owning them. Overlay or eject any name in either list.
...
- *"Warm up kage's palette for both renditions"* - the agent lists the manifest with `cherry theme.tokens`, overrides rung-1 tokens, and verifies with `cherry check`.
...
The one rule to hold an agent to: never edit the theme's own files under `themes/kage/` unless they were ejected there on purpose - `cherry theme.diff` will name anything that drifts.
