...
<p class="hero-tagline">A modern take on Octopress: typed content, deterministic builds, themes that survive upgrades — all in one binary with the fuse lit.</p>
...
One file, valid frontmatter, `draft: true`. The envelope hands back the path — pipe it straight into your editor, a script, or whatever else does your writing.

### 2 · Verify

`cherry check` builds the whole site in memory — it writes nothing — and returns structured diagnostics. Here it catches a link to a page that doesn't exist:
...
<p class="wall-line"><strong>Markdown all the way out.</strong> Every route ships its markdown beside the HTML, plus <a href="/llms.txt">/llms.txt</a> — curl-able and grep-able.</p>
