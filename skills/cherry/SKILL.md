...
The same text is available in the terminal: `cherry help` lists every verb with a summary, and `cherry help VERB` (or `cherry VERB --help`) prints one verb's doc — rendered from the same single source as this reference, so the two never disagree.
