defmodule Cherry.SylvaThemeTest do
  ...
  test "sylva is an official theme (contract 1.1) and passes conformance" do
    assert "sylva" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("sylva"))
    assert theme.name == "sylva"
  ...
  test "sylva ships layout, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("sylva"))
  ...
  @tag :tmp_dir
  test "theme: \"sylva\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "sylva"])
    )
    ...
    assert css =~ "sylva theme"
    assert css =~ "--color-accent: light-dark(#3f6b2e, #9ccc7a)"

    assert File.exists?(Path.join(out, "assets/fonts/newsreader-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/fonts/newsreader-italic-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/sylva-season.js"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"sylva-season\""
