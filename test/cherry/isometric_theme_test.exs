defmodule Cherry.IsometricThemeTest do
  ...
  test "isometric is an official theme (contract 1.1) and passes conformance" do
    assert "isometric" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("isometric"))
    assert theme.name == "isometric"
  ...
  test "isometric ships no templates — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("isometric"))
  ...
  @tag :tmp_dir
  test "theme: \"isometric\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "isometric"])
    )
    ...
    assert css =~ "isometric theme"
    assert css =~ "--color-accent: light-dark(#0f766e, #2dd4bf)"

    assert File.exists?(Path.join(out, "assets/fonts/sora-latin.woff2"))
