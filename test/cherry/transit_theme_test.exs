defmodule Cherry.TransitThemeTest do
  ...
  test "transit is an official theme (contract 1.1) and passes conformance" do
    assert "transit" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("transit"))
    assert theme.name == "transit"
  ...
  test "transit ships layout, post, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("transit"))
    site = %Site{title: "t", url: "https://x", base_path: "", root: ".", output: "_s"}
    shipped = [:layout, :post, :post_list]
  ...
  @tag :tmp_dir
  test "theme: \"transit\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "transit"])
    )
    ...
    assert css =~ "transit theme"
    assert css =~ "--color-accent: light-dark(#0b5fff, #6ea8ff)"

    assert File.exists?(Path.join(out, "assets/fonts/gabarito-latin.woff2"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"transit-progress\""
