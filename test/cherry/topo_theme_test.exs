defmodule Cherry.TopoThemeTest do
  ...
  test "topo is an official theme (contract 1.1) and passes conformance" do
    assert "topo" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("topo"))
    assert theme.name == "topo"
  ...
  test "topo ships only its layout — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("topo"))
  ...
  @tag :tmp_dir
  test "theme: \"topo\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "topo"])
    )
    ...
    assert css =~ "topo theme"
    assert css =~ "--color-accent: light-dark(#b5451b, #e0895a)"

    assert File.exists?(Path.join(out, "assets/fonts/alegreya-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/fonts/alegreya-italic-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/fonts/alegreya-sans-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/fonts/alegreya-sans-bold-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/topo-relief.js"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"topo-relief\""
    assert index =~ "class=\"topo-scale\""
