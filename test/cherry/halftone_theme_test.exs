defmodule Cherry.HalftoneThemeTest do
  ...
  test "halftone is an official theme (contract 1.1) and passes conformance" do
    assert "halftone" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("halftone"))
    assert theme.name == "halftone"
  ...
  test "halftone ships only its post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("halftone"))
  ...
  @tag :tmp_dir
  test "theme: \"halftone\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "halftone"])
    )
    ...
    assert css =~ "halftone theme"
    assert css =~ "--color-accent: light-dark(#e63b2e, #ff6b5e)"

    assert File.exists?(Path.join(out, "assets/fonts/archivo-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/fonts/archivo-expanded-latin.woff2"))
