defmodule Cherry.AtlasThemeTest do
  ...
  test "atlas is an official theme (contract 1.1) and passes conformance" do
    assert "atlas" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("atlas"))
    assert theme.name == "atlas"
  ...
  test "atlas ships layout, page, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("atlas"))
  ...
  @tag :tmp_dir
  test "theme: \"atlas\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "atlas"])
    )
    ...
    assert css =~ "atlas theme"
    assert css =~ "--color-accent: light-dark(#0b6e6e, #5ad1c4)"

    assert File.exists?(Path.join(out, "assets/fonts/geologica-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/atlas-map.js"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"sheet sheet-home\""
