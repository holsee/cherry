defmodule Cherry.BellowsThemeTest do
  ...
  test "bellows is an official theme (contract 1.1) and passes conformance" do
    assert "bellows" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("bellows"))
    assert theme.name == "bellows"
  ...
  test "bellows ships only its page — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("bellows"))
  ...
  @tag :tmp_dir
  test "theme: \"bellows\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "bellows"])
    )
    ...
    assert css =~ "bellows theme"
    assert css =~ "--color-accent: light-dark(#d7431b, #ff7a4d)"

    assert File.exists?(Path.join(out, "assets/fonts/anybody-latin.woff2"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"bellows-word\""
