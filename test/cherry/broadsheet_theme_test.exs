defmodule Cherry.BroadsheetThemeTest do
  ...
  test "broadsheet is an official theme (contract 1.1) and passes conformance" do
    assert "broadsheet" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("broadsheet"))
    assert theme.name == "broadsheet"
  ...
  test "broadsheet ships layout, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("broadsheet"))
  ...
  @tag :tmp_dir
  test "theme: \"broadsheet\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "broadsheet"])
    )
    ...
    assert css =~ "broadsheet theme"
    assert css =~ "--color-accent: light-dark(#8a1c1c, #e36b6b)"

    assert File.exists?(Path.join(out, "assets/fonts/playfair-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/fonts/playfair-italic-latin.woff2"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"site-header masthead\""
