defmodule Cherry.FlipdotThemeTest do
  ...
  test "flipdot is an official theme (contract 1.1) and passes conformance" do
    assert "flipdot" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("flipdot"))
    assert theme.name == "flipdot"
  ...
  test "flipdot ships layout, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("flipdot"))
  ...
  @tag :tmp_dir
  test "theme: \"flipdot\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "flipdot"])
    )
    ...
    assert css =~ "flipdot theme"
    assert css =~ "--color-accent: light-dark(#b2570a, #ffb020)"

    assert File.exists?(Path.join(out, "assets/fonts/doto-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/flipdot-board.js"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "data-flip"
