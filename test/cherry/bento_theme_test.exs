defmodule Cherry.BentoThemeTest do
  ...
  test "bento is an official theme (contract 1.1) and passes conformance" do
    assert "bento" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("bento"))
    assert theme.name == "bento"
  ...
  test "bento ships layout, page, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("bento"))
  ...
  @tag :tmp_dir
  test "theme: \"bento\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "bento"])
    )
    ...
    assert css =~ "bento theme"
    assert css =~ "--color-accent: light-dark(#0066cc, #2997ff)"

    assert File.exists?(Path.join(out, "assets/fonts/onest-latin.woff2"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"bento bento-home\""
    assert index =~ "class=\"bento bento-nav\""
