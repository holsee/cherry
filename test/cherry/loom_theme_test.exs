defmodule Cherry.LoomThemeTest do
  ...
  test "loom is an official theme (contract 1.1) and passes conformance" do
    assert "loom" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("loom"))
    assert theme.name == "loom"
  ...
  test "loom ships layout, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("loom"))
  ...
  @tag :tmp_dir
  test "theme: \"loom\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "loom"])
    )
    ...
    assert css =~ "loom theme"
    assert css =~ "--color-accent: light-dark(#a8322b, #e8705f)"

    assert File.exists?(Path.join(out, "assets/fonts/young-serif-latin.woff2"))
    ...
    assert File.exists?(Path.join(out, "assets/loom-cloth.js"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"loom-band\""
