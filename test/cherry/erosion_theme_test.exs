defmodule Cherry.ErosionThemeTest do
  ...
  test "erosion is an official theme (contract 1.1) and passes conformance" do
    assert "erosion" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("erosion"))
    assert theme.name == "erosion"
  ...
  test "erosion ships layout, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("erosion"))
    site = %Site{title: "t", url: "https://x", base_path: "", root: ".", output: "_s"}
    shipped = [:layout, :post_list]
  ...
  @tag :tmp_dir
  test "theme: \"erosion\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "erosion"])
    )
    ...
    assert css =~ "erosion theme"
    assert css =~ "--color-accent: light-dark(#2e7264, #6fcfb6)"

    assert File.exists?(Path.join(out, "assets/fonts/fraunces-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/fonts/fraunces-italic-latin.woff2"))
    assert File.exists?(Path.join(out, "assets/erosion-grains.js"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"erosion-grains\""
