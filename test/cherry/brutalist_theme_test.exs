defmodule Cherry.BrutalistThemeTest do
  ...
  test "brutalist is an official theme (contract 1.1) and passes conformance" do
    assert "brutalist" in Theme.builtin_names()
    assert {:ok, theme} = Theme.load(Theme.builtin_root("brutalist"))
    assert theme.name == "brutalist"
  ...
  test "brutalist ships layout, post_list — the rest inherit from the framework" do
    {:ok, theme} = Theme.load(Theme.builtin_root("brutalist"))
  ...
  @tag :tmp_dir
  test "theme: \"brutalist\" swaps the world without touching content", %{tmp_dir: tmp} do
    ...
    File.write!(
      ...
      ~s([title: "Folio", url: "https://folio.example", theme: "brutalist"])
    )
    ...
    assert css =~ "brutalist theme"
    assert css =~ "--color-accent: light-dark(#0000ee, #ffe600)"

    assert File.exists?(Path.join(out, "assets/fonts/anton-latin.woff2"))
    index = File.read!(Path.join(out, "index.html"))
    assert index =~ "class=\"site-header slab\""
